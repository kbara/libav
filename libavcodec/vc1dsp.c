...
static void vc1_v_s_overlap_c(int16_t *top, int16_t *bottom)
{
    ...
    for (i = 0; i < 8; i++) {
        ...
        top[48]   = ((a * (1 << 3)) - d1 + rnd1) >> 3;
        top[56]   = ((b * (1 << 3)) - d2 + rnd2) >> 3;
        bottom[0] = ((c * (1 << 3)) + d2 + rnd1) >> 3;
        bottom[8] = ((d * (1 << 3)) + d1 + rnd2) >> 3;
...
static void vc1_h_s_overlap_c(int16_t *left, int16_t *right)
{
    ...
    for (i = 0; i < 8; i++) {
        ...
        left[6]  = ((a * (1 << 3)) - d1 + rnd1) >> 3;
        left[7]  = ((b * (1 << 3)) - d2 + rnd2) >> 3;
        right[0] = ((c * (1 << 3)) + d2 + rnd1) >> 3;
        right[1] = ((d * (1 << 3)) + d1 + rnd2) >> 3;
