                               ...
                               const uint8_t *data, int size, int *ref)
{
    ...
    if (!s->keyframe && !s->intraonly) {
        ...
        for (i = 0; i < 3; i++)
            if (vp56_rac_get_prob_branchy(&s->c, 252))
                s->prob.p.mv_joint[i] = (vp8_rac_get_uint(&s->c, 7) * (1 << 1)) | 1;

        for (i = 0; i < 2; i++) {
            if (vp56_rac_get_prob_branchy(&s->c, 252))
                s->prob.p.mv_comp[i].sign =
                    (vp8_rac_get_uint(&s->c, 7) * (1 << 1)) | 1;
                           ...
                           ptrdiff_t yoff, ptrdiff_t uvoff, enum BlockLevel bl)
{
    ...
    int c = ((s->above_partition_ctx[col]       >> (3 - bl)) & 1) |
            (((s->left_partition_ctx[row & 0x7] >> (3 - bl)) & 1) * (1 << 1));
    int ret;
        ...
        for (x = 1; vm & ~(x - 1); x <<= 2, ptr += 16, l += 2) {
            if (row || y) {
                if (vm & x) {
                    ...
                    s->dsp.loop_filter_8[!!(vmask[1] & (x * (1 << 1)))]
                                        [1](ptr + 8, ls_y, E, I, H);
                }
            }
            if (vm3 & x) {
            ...
            for (x = 1; vm & ~(x - 1); x <<= 4, ptr += 16, l += 4) {
                if (row || y) {
                    if (vm & x) {
                        ...
                        s->dsp.loop_filter_8[!!(vmask[1] & (x * (1 << 2)))]
                                            [1](ptr + 8, ls_uv, E, I, H);
                    }
                }
            }
